...
export const getTournaments = async (input?: z.infer<typeof inputParams>) => {
	await adminGuard()
...
export const getSports = async (input?: z.infer<typeof inputParams>) => {
	await adminGuard()
	const { page = 1, limit = 10, search } = input || {}
	const offset = (page - 1) * limit
...
export const getTeamsBySportId = async (
	input: z.infer<typeof getTeamsBySportIdParams>,
) => {
	await adminGuard()
	const { page = 1, limit = 10, search } = input || {}
	const offset = (page - 1) * limit
...
export const getWeeksByTournamentId = async (
	input: z.infer<typeof getWeeksByTournamentIdParams>,
) => {
	await adminGuard()
...
export const getUsers = async (input?: z.infer<typeof getUsersInputParams>) => {
	await adminGuard()
	const { page = 1, limit = 10, search, kind } = input || {}
	const offset = (page - 1) * limit
	const where = search
		? and(
				kind !== 'all' ? eq(users.suspended, kind === 'suspended') : undefined,
